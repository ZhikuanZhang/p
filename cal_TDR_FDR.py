...
def caculate(samplePath,locPath):
  pores = []
  detections = []
  for index in os.listdir(samplePath):
    a, b = os.path.splitext(index);
    pores_path = os.path.join(samplePath, a + '.txt')
    index_pores = load_pores_txt(pores_path)
    pores.append(np.array(index_pores))

    dets_path = os.path.join(locPath, a + '.txt')
    index_dets = load_pores_txt(dets_path)
    detections.append(np.array(index_dets))
...
def caculateOne(samplePath,locPath):
  pores = []
  detections = []
  pores_path=samplePath
  index_pores = load_pores_txt(pores_path)
  pores.append(np.array(index_pores))
  dets_path = locPath
  index_dets = load_pores_txt(dets_path)
